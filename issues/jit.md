...
- **Concern:** `defaultdict.__getitem__` with a missing key calls the
  ...
  [device-caches-race.md](jit/device-caches-race.md).
...
Written up in [used-global-vals-unsynchronized-read.md](jit/used-global-vals-unsynchronized-read.md).

### 7. `JITFunction.pre_run_hooks`

Written up in [pre-run-hooks-unsynchronized-iteration.md](jit/pre-run-hooks-unsynchronized-iteration.md).
...
[hash-placeholder-not-a-bug.md](jit/hash-placeholder-not-a-bug.md) — no direct
