...
- **Code (`RedisRemoteCacheBackend.__init__`):**
  ```python
  ...
  ```
  And `RemoteCacheManager.__init__` reads
...
- **Code (`triton_key`):**
  ...
  def triton_key():
      ...
      ...
  ```
- **Concern:** Called from `get_cache_key` and from `autotuner.py`'s
  `Autotuner.__init__`. Two threads that reach `triton_key()` on a cold
...
- **Orthogonal note:** `triton_key()` reads files from the Triton
  install tree. If those files are modified *during
...
- **Code (`FileCacheManager.has_file` / `get_file`):**
  ```python
...
- **Concern:** The returned path can disappear between
  ...
  - `build.py` handles the race explicitly — wraps the load
    in `try: ... except (RuntimeError, ImportError): warn(...)` and
    continues to a fresh build. Safe.
  - `compiler.py` uses `get_group` instead of `get_file` for the
    metadata sidecar, so the TOCTOU window is closed by atomic replace
    of the group file.
  - `compiler.py` `fn_override_manager.get_file(ir_filename)` reads a
    user-placed override file; if that file vanishes between
    `get_file` and the subsequent `parse(full_name, ...)`, `parse()`
    raises. Low probability (override files are typically stable),
    but possible.
  - `autotuner.py` opens the file inside a plain `with open`
...
- **`FileCacheManager.__init__` calling `os.makedirs(cache_dir,
  exist_ok=True)`**. Idempotent; `exist_ok=True`
...
- **`FileCacheManager.put` `assert self.lock_path is not None`.**
  `lock_path` is only non-None when the manager was
...
- **`FileCacheManager.get_group` child-path existence checks.**
  The group file is the last thing written in a
...
- **`make_so_cache_key` and `get_cache_key` purity.** Both are pure
  functions over their arguments; no module state.
...
- **`_base32` purity.** Pure function; no state.
...
- **`get_cache_manager` / `get_override_manager` / `get_dump_manager`
  each constructing a fresh `FileCacheManager` per call.**
